...
def es_memoria_nula(matriz, tol=1e-9):
    """
    Devuelve True si la fuente es de memoria nula,
    es decir, si todas las filas de la matriz son iguales (dentro de tol).
    """
    n = len(matriz)
    referencia = matriz[0]
    for i in range(1, n):
        for j in range(len(referencia)):
            if abs(matriz[i][j] - referencia[j]) > tol:
                return False
    return True
...
print("¿Fuente de memoria nula?:", es_memoria_nula(matriz, tol= 0.1))
